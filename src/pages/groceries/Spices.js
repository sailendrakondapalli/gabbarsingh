...
class Spices extends React.Component {
  ...
  componentDidMount() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error("❌ Geolocation error in Spices:", err);
  ...
  fetchProductsFromLocation = async (position) => {
    ...
    console.log("📍 Spices GPS:", latitude, longitude);
    ...
    try {
      ...
      console.log("🍛 Nearby cities for Spices:", nearbyCities);
      ...
      console.error("❌ Spices location fetch failed:", err);
        this.setState({ loading: false }); 
  ...
  fallbackToIPorLocalStorage = () => {
    fetch("https://ipapi.co/json/")
      ...
      .then((locationData) => {
        ...
        if (city) {
          console.log("🌐 IP-based city for Spices:", city);
          const user = JSON.parse(localStorage.getItem("user")) || {};
          user.city = city;
          localStorage.setItem("user", JSON.stringify(user));
  ...
  <p>Loading products near you...</p>
        ...
        <p style={{ textAlign: "center", color: "red" }}>
          🚫 No Spices products found near you{userCity ? ` in ${userCity}` : ""}.
        </p>
      )}
    </div>
  );
