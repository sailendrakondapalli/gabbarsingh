...
class Bakery extends React.Component {
  ...
  componentDidMount() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error("❌ Geolocation error in Bakery:", err);
  ...
  fetchProductsFromLocation = async (position) => {
    ...
    console.log("📍 Bakery GPS:", latitude, longitude);
    ...
    try {
      ...
      console.log("🍛 Nearby cities for Bakery:", nearbyCities);
      ...
      console.error("❌ Bakery location fetch failed:", err);
        this.setState({ loading: false }); 
  ...
  fallbackToIPorLocalStorage = () => {
    fetch("https://ipapi.co/json/")
      ...
      .then((locationData) => {
        ...
        if (city) {
          console.log("🌐 IP-based city for Bakery:", city);
          const user = JSON.parse(localStorage.getItem("user")) || {};
          user.city = city;
          localStorage.setItem("user", JSON.stringify(user));
  ...
  <p>Loading products near you...</p>
        ...
        <p style={{ textAlign: "center", color: "red" }}>
          🚫 No Bakery products found near you{userCity ? ` in ${userCity}` : ""}.
        </p>
      )}
    </div>
  );
