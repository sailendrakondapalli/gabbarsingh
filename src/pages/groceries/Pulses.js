...
class Pulses extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      topProducts: [],
  ...
  componentDidMount() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error("❌ Geolocation error in Pulses:", err);
  ...
  fetchProductsFromLocation = async (position) => {
    ...
    console.log("📍 Pulses GPS:", latitude, longitude);
    ...
    try {
      ...
      console.log("🍛 Nearby cities for Pulses:", nearbyCities);
      ...
      console.error("❌ Pulses location fetch failed:", err);
        this.setState({ loading: false }); 
  ...
  fallbackToIPorLocalStorage = () => {
    fetch("https://ipapi.co/json/")
      ...
      .then((locationData) => {
        ...
        if (city) {
          console.log("🌐 IP-based city for Pulses:", city);
          const user = JSON.parse(localStorage.getItem("user")) || {};
          user.city = city;
          localStorage.setItem("user", JSON.stringify(user));
  ...
  <p>Loading products near you...</p>
        ...
        <p style={{ textAlign: "center", color: "red" }}>
          🚫 No Pulses products found near you{userCity ? ` in ${userCity}` : ""}.
        </p>
      )}
    </div>
  );
