...
class Topwere extends React.Component {
  ...
  componentDidMount() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error("❌ Geolocation error in Topwere:", err);
  ...
  fetchProductsFromLocation = async (position) => {
    ...
    console.log("📍 Topwere GPS:", latitude, longitude);
    ...
    try {
      ...
      console.log("🍛 Nearby cities for Topwere:", nearbyCities);
      ...
      console.error("❌ Topwere location fetch failed:", err);
        this.setState({ loading: false }); 
  ...
  fallbackToIPorLocalStorage = () => {
    fetch("https://ipapi.co/json/")
      ...
      .then((locationData) => {
        ...
        if (city) {
          console.log("🌐 IP-based city for Topwere:", city);
          const user = JSON.parse(localStorage.getItem("user")) || {};
          user.city = city;
          localStorage.setItem("user", JSON.stringify(user));
  ...
  <p>Loading products near you...</p>
        ...
        <p style={{ textAlign: "center", color: "red" }}>
          🚫 No Topwere products found near you{userCity ? ` in ${userCity}` : ""}.
        </p>
      )}
    </div>
  );
