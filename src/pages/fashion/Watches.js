...
class Watches extends React.Component {
  ...
  componentDidMount() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error("❌ Geolocation error in Watches:", err);
  ...
  fetchProductsFromLocation = async (position) => {
    ...
    console.log("📍 Watches GPS:", latitude, longitude);
    ...
    try {
      ...
      console.log("🍛 Nearby cities for Watches:", nearbyCities);
      ...
      console.error("❌ Watches location fetch failed:", err);
        this.setState({ loading: false }); 
  ...
  fallbackToIPorLocalStorage = () => {
    fetch("https://ipapi.co/json/")
      ...
      .then((locationData) => {
        ...
        if (city) {
          console.log("🌐 IP-based city for Watches:", city);
          const user = JSON.parse(localStorage.getItem("user")) || {};
          user.city = city;
          localStorage.setItem("user", JSON.stringify(user));
  ...
  <p>Loading products near you...</p>
        ...
        <p style={{ textAlign: "center", color: "red" }}>
          🚫 No Watches products found near you{userCity ? ` in ${userCity}` : ""}.
        </p>
      )}
    </div>
  );
