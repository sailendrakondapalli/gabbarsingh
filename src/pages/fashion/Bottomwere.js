...
class Bottomwere extends React.Component {
  ...
  componentDidMount() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error("❌ Geolocation error in Bottomwere:", err);
  ...
  fetchProductsFromLocation = async (position) => {
    ...
    console.log("📍 Bottomwere GPS:", latitude, longitude);
    ...
    try {
      ...
      console.log("🍛 Nearby cities for Bottomwere:", nearbyCities);
      ...
      console.error("❌ Bottomwere location fetch failed:", err);
        this.setState({ loading: false }); 
  ...
  fallbackToIPorLocalStorage = () => {
    fetch("https://ipapi.co/json/")
      ...
      .then((locationData) => {
        ...
        if (city) {
          console.log("🌐 IP-based city for Bottomwere:", city);
          const user = JSON.parse(localStorage.getItem("user")) || {};
          user.city = city;
          localStorage.setItem("user", JSON.stringify(user));
  ...
  fetchBiriyaniProducts = async (cities, primaryCity) => {
    try {
      ...
      const filtered = allProducts.filter(
        (p) =>
          p.category &&
          normalize(p.category) === "bottomwere"
      );
  ...
  <p>Loading products near you...</p>
        ...
        <p style={{ textAlign: "center", color: "red" }}>
          🚫 No Bottomwere products found near you{userCity ? ` in ${userCity}` : ""}.
        </p>
      )}
    </div>
  );
